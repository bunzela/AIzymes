...
class MLP(nn.Module):
    """A simple multi-layer perceptron (MLP) model with dropout."""
    def __init__(self, input_size, n_layers, n_features, dropout_prob=0.5):
        super(MLP, self).__init__()

        layers = []
        for layer in range(n_layers):
            in_features = input_size if layer == 0 else n_features[layer-1]
            out_features = n_features[layer]
